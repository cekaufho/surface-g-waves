...
module config_module
 use main_module
 real*8 :: fac=1,p=2.,mix=2e-06,L0,H0,B0,l=4.
...
subroutine set_grid
 ...
 H0 = dzt(4)*nz    ! Height of the tank 
...
! dimensions you want to allocate to, i.e. how much space
 u_source=u0* sin (2*1* 3.14 * OM0 * itt * dt_tracer ) ! changed pi to 3.14
...
do i = is_pe, ie_pe
    do j = js_pe, je_pe
        if (i == 40*p .AND. j .LE. int((2/5)*ny )) kbot(i,j) =0
        !if (i== 40*p .AND. j .GE. ((2/5)*ny)+(p*2) .AND. j .LE. (((3/5)*ny)-(p*2))) kbot(i,j) =0
        if (i ==40*p .AND. j .GE. int((3/5)*ny)) kbot(i,j)=0
    enddo
enddo
