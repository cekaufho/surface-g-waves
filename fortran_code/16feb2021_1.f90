...
module config_module
 use main_module
 real*8 :: fac=1,p=2.,mix=2e-12,L0,H0,B0,l=1.
...
u_source=u0*1e+0* sin (2*1E+0* 3.14 * OM0 * itt * dt_tracer ) ! changed pi to 3.14
...
do i = is_pe, ie_pe
    do j = js_pe, je_pe
		if (p .GE. 2) k=p-1
        if (i == 40*p .AND. j .LE. ((m-9-3+1)*p)-(p-1)+k ) kbot(i,j) =0
        if (i== 40*p .AND. j .GE. ((m-9+1)*p)-(p-1) .AND. j .LE. (m+9)*p )  kbot(i,j) =0
        if (i ==40*p .AND. j .GE. (m+9+3)*p-k) kbot(i,j)=0
    enddo
enddo
