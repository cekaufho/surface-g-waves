...
subroutine set_parameter
 ...
 implicit none
  ...
  runlen = 2500
  enable_diag_ts_monitor = .true.; ts_monint = 0.5
  enable_diag_snapshots  = .true.; snapint  = 50
...
subroutine set_grid
 ...
 dxt(:) = 462.9 !Single Slit: 299.9
 dyt(:) = 344.5 !Single Slit: 223.1
 dzt(:) = 0.1
...
subroutine set_initial_conditions
 ...
 ! Specify here where you want the wavemaker
  ...
  do j = js_pe, je_pe
   do i = is_pe, ie_pe
    u0(i, j, 2) = maskU(i, j, 2) * 1./(100 * 60.0 * dt_tracer) * exp( -(xu(i)-x0)**2/(dxt(is_pe)*1)**2 )
	enddo
  enddo
...
subroutine set_forcing
 ...
 !Wavemaker
 u_source = u0 * sin (1 * 2 * pi * OM0 * itt * dt_tracer )
...
do i = is_pe, ie_pe
    do j = js_pe, je_pe
        if (i == 10 .AND. j .LE. 29 ) kbot(i, j) = 0
        if (i== 10 .AND. j .GE. 29+6 .AND. j .LE. 29+6+30) kbot(i, j) = 0
        if (i == 10 .AND. j .GE. 29+6+30+6) kbot(i, j) = 0
    enddo
enddo
...
!do i = is_pe, ie_pe
!    do j = js_pe, je_pe
!        if (i == 67 .AND. j .LE. 46 ) kbot(i,j) = 0
!                if (i == 67 .AND. j .GE. 46+8 ) kbot(i,j) = 0
!    enddo
!enddo
